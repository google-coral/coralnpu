...
async def run(fixture, x, kernel_id=0, alias=0, scratch_len=None, repeat=1):
    ...
    await fixture.write(
        "active_scratch_len", np.array([scratch_len], dtype=np.uint32)
    )
    ...
    await fixture.write(
        "softmax_out", np.full(n + pad, CANARY, dtype=np.uint16)
    )
...
async def softmax_bf16_corr_test(dut):
    ...
    for kid, name in VARIANTS:
        ...
        cases = {
            ...
            "one-hot":
            np.concatenate(([30.0], np.zeros(n - 1))).astype(np.float32),
    ...
    for m in [1, 2, 8, 16, 31, 32]:
        for x in (
            ...
            (np.concatenate(([40.0], np.zeros(m - 1))).astype(np.float32)
             if m > 1 else np.array([3.0], np.float32)),
        ):
    ...
    log.info(
        f"softmax [small-row]: worst {worst} ULP, bit-identical to 3-pass"
    )

    ok_s, out_s, _, _ = await run(
        fixture,
        rng.uniform(-10.0, 10.0, n).astype(np.float32), SMALL_ROW_KID
    )
    ...
    assert ok_s == 1, "small-row must run without scratch"
    check(out_s, golden, "small-row zero-scratch")
