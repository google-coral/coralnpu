...
// -x negation into the range reduction) and differ ONLY in the final divide:
//  - SigmoidBf16               1/(1+e) via vfrdiv (correctly rounded)
...
// reciprocal via vfrec7 + one Newton-Raphson step. denom = 1 + exp(-x)
...
// reciprocal via vfrec7 with no Newton refinement. Same positive-normal precondition.
