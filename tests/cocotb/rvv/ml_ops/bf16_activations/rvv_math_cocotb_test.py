...
MAXKEY_SPECIALS = np.array([
    0x0000, 0x8000, 0x7F80, 0xFF80, 0x4000, 0xC000, 0x3F80, 0xBF80
],
                           dtype=np.uint16)
...
def make_narrow_inputs(rng, n):
    ...
    k = max(1, n // 3)
    u[:k] = (u[:k] & np.uint32(0xFFFF0000)) | np.uint32(
        0x8000
    )  # tie, keep-bit varies
    u[k:2 *
      k] = (u[k:2 * k]
            & np.uint32(0xFFFE0000)) | np.uint32(0x8000)  # tie, keep-bit even
...
async def rvv_math_test(dut):
    ...
    for n in [1, 31, 33, 256, 643,
              4096]:  # includes non-multiples of the 32-lane group
        ...
        exp_in[:min(7, n)] = np.array([-100, -88, -87.9, 0, 87.9, 88, 100],
                                      np.float32)[:min(7, n)]
        expnp_in = -np.abs(exp_in).astype(
            np.float32
        )  # nonpos form needs x <= 0
        expnp_in[:min(4, n)] = np.array([0, -0.0, -88, -100],
                                        np.float32)[:min(4, n)]
        recip_in = np.exp(rng.uniform(np.log(1e-3), np.log(1e3),
                                      n)).astype(np.float32)
        recip_in[:min(8, n)] = np.array([
            1.0, 1.0000061, 2.0, 0.5, 1e-3, 1e3, 1e-6, 1e6
        ], np.float32)[:min(8, n)]
        widen_in = rng.integers(0, 1 << 16, n, dtype=np.uint16)
        widen_in[:min(BF16_SPECIALS.
                      size, n)] = BF16_SPECIALS[:min(BF16_SPECIALS.size, n)]
        ...
        maxkey_in = f32_to_bf16(
            rng.uniform(-100.0, 100.0, n).astype(np.float32)
        )
        maxkey_in[:min(MAXKEY_SPECIALS.size, n)
                  ] = MAXKEY_SPECIALS[:min(MAXKEY_SPECIALS.size, n)]
        ...
        np.testing.assert_array_equal((await
                                       fixture.read("cvt_out",
                                                    n * 2)).view(np.uint16),
                                      cvt)
        ...
        if n >= 256:
            ...
            assert (rw < HALF_ULP), f"exp rel {rw:.2e} >= {HALF_ULP:.1e}"
            log.info(
        ...
        np.testing.assert_allclose(
            ts, treesum_in.astype(np.float64).sum(), rtol=5e-5
        )
        ...
        no = wb[is_nan]
        assert np.all((((no >> np.uint32(23)) & np.uint32(0xFF)) == 0xFF)
